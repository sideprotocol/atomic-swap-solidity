...
task("deploy:inter-chain:setTrust", "set trust remote contract address")
  .addOptionalParam("f", "force write")
  .setAction(async ({ f }, { ethers, upgrades, network }) => {
    // const interChainContractAddress =
    //   Settings[`interChainAtomicSwap${network.name}` as keyof typeof Settings];
    // const interChainAtomicSwap = SideLzAppUpgradable__factory.connect(
    //   interChainContractAddress
    // );
    // interChainAtomicSwap.
    // const admin = process.env.ADMIN;
    // const treasury = process.env.TREASURY;
